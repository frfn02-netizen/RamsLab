...
vi.mock("@/components/providers/auth-providers", () => ({
  useAuth: () => ({ user: { role: mocks.auth.role }, status: "authenticated" }),
}));

const dosen = {
  _id: "dosen-1",
  fullName: "Dr. Sea Lecturer",
  specialization: ["Reliability"],
  isPublic: true,
};
const student = {
  _id: "student-1",
  fullName: "Marine Student",
...
afterEach(() => {
  cleanup();
  mocks.getDosenList.mockReset();
  mocks.deleteDosen.mockReset();
  mocks.getStudentList.mockReset();
...
describe("Admin People list deletion", () => {
  it("deletes the selected Dosen after confirmation", async () => {
    mocks.getDosenList.mockResolvedValue([dosen]);
    mocks.deleteDosen.mockResolvedValue(undefined);
    render(<DosenPage />);
    await waitFor(() =>
      expect(screen.getByText("Dr. Sea Lecturer")).toBeInTheDocument(),
    );
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));
    expect(screen.getByRole("dialog")).toHaveTextContent("Dr. Sea Lecturer");
    fireEvent.click(
      screen.getByRole("dialog").querySelector("button.bg-red-700")!,
    );
    await waitFor(() =>
      expect(mocks.deleteDosen).toHaveBeenCalledWith("dosen-1"),
    );
    expect(screen.queryByText("Dr. Sea Lecturer")).not.toBeInTheDocument();
  });
  ...
  it("does not delete a Student when confirmation is cancelled", async () => {
    ...
    render(<StudentsPage />);
    await waitFor(() =>
      expect(screen.getByText("Marine Student")).toBeInTheDocument(),
    );
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));
  ...
  it("keeps a Student row when deletion fails", async () => {
    mocks.getStudentList.mockResolvedValue([student]);
    mocks.deleteStudent.mockRejectedValue(new Error("Delete failed"));
    render(<StudentsPage />);
    await waitFor(() =>
      expect(screen.getByText("Marine Student")).toBeInTheDocument(),
    );

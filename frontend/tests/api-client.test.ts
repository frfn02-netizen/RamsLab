...
describe("API client", () => {
  it("includes credentials and sends CSRF only for mutations", async () => {
    ...
    await apiRequest("/write", { method: "PATCH", body: JSON.stringify({ value: 1 }) });
    ...
    expect((safeOptions.headers as Headers).get("Accept")).toBe("application/json");
  ...
  it("parses safe server errors and notifies the auth layer on 401", async () => {
    ...
    await expect(apiRequest("/dashboard")).rejects.toMatchObject({ status: 401, message: "Session expired" });
  ...
  it("normalizes network failures into ApiError", async () => {
    ...
    await expect(apiRequest("/public/projects")).rejects.toBeInstanceOf(ApiError);
    await expect(apiRequest("/public/projects")).rejects.toMatchObject({ status: 0 });
  });
});

...
const { getAlumniList, deleteAlumni, auth } = vi.hoisted(() => ({ getAlumniList: vi.fn(), deleteAlumni: vi.fn(), auth: { role: "DOSEN" as string } }));
vi.mock("@/lib/api/alumni", () => ({ getAlumniList, deleteAlumni }));
vi.mock("@/components/providers/auth-providers", () => ({ useAuth: () => ({ user: { role: auth.role }, status: "authenticated" }) }));
...
describe("Alumni list", () => {
  it("confirms and deletes the selected alumni", async () => {
    ...
    expect(screen.getByRole("link", { name: "View" })).toHaveAttribute("href", "/dashboard/alumni/alumni-1");
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));
    expect(screen.getByRole("dialog")).toHaveTextContent("Marine Researcher");
    fireEvent.click(screen.getByRole("dialog").querySelector("button.bg-red-700")!);
    ...
    expect(screen.getByRole("status")).toHaveTextContent("Marine Researcher was deleted successfully.");
  });

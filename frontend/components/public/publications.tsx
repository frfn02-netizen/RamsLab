...
import { PublicError } from "./public-states";
...
function PublicationCard({ project, t, active, staged = false }: { project: PublicationProject; t: (key: string) => string; active: boolean; staged?: boolean }) {
  ...
  return <article className={`group border bg-white p-6 transition-[opacity,transform,border-color,box-shadow] duration-500 motion-reduce:translate-y-0 motion-reduce:opacity-100 motion-reduce:transition-none sm:p-7 ${staged && !active ? "pointer-events-none translate-y-3 border-[var(--border)] opacity-0" : active ? "translate-y-0 border-[var(--rams-red)]/55 opacity-100 shadow-[0_10px_28px_rgba(11,32,56,0.07)]" : "translate-y-3 border-[var(--border)] opacity-70"} hover:-translate-y-0.5 hover:border-[var(--rams-red)]/55 hover:shadow-[0_10px_28px_rgba(11,32,56,0.07)]`}>
    ...
    <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-[var(--border)] pt-5">
      {publicationUrl && <a href={publicationUrl} target="_blank" rel="noreferrer" className="text-xs font-bold tracking-[0.08em] text-[var(--rams-red)] transition hover:text-[var(--rams-red-dark)]" aria-label={`${t("viewPdf")}: ${project.title}`}>{t("viewPdf")} <span aria-hidden="true">↗</span></a>}
...
function PublicationSkeleton() {
  return <div className="animate-pulse border border-[var(--border)] bg-white p-6 sm:p-7" aria-hidden="true"><div className="flex justify-between"><div className="h-3 w-28 bg-[var(--border)]" /><div className="h-6 w-12 bg-[var(--border)]" /></div><div className="mt-7 h-7 w-4/5 bg-[var(--border)]" /><div className="mt-4 h-4 w-3/5 bg-[var(--border)]" /><div className="mt-3 h-4 w-2/5 bg-[var(--border)]" /><div className="mt-7 border-t border-[var(--border)] pt-5"><div className="h-3 w-36 bg-[var(--border)]" /></div></div>;
}

const PUBLIC_HEADER_HEIGHT_PX = 80;

function useStickyPublicationProgress(sectionRef: RefObject<HTMLDivElement | null>, stickyViewportRef: RefObject<HTMLDivElement | null>, streamViewportRef: RefObject<HTMLDivElement | null>, streamRef: RefObject<HTMLDivElement | null>, count: number) {
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      const scrollDistance = Math.max(section.offsetHeight - stickyViewport.offsetHeight, 1);
      const sectionTop = section.getBoundingClientRect().top;
      const progress = Math.min(1, Math.max(0, (PUBLIC_HEADER_HEIGHT_PX - sectionTop) / scrollDistance));
      const nextOffset = streamTravelRef.current * progress;
    ...
    const measureFrame = window.requestAnimationFrame(measureStream);
    return () => {
  ...
  }, [count, sectionRef, stickyViewportRef, streamViewportRef, streamRef]);

  return { activeIndex, streamOffset, streamTravel };
}

function PublicationTimeline({ groups, t, activeIndex, streamViewportRef, streamRef, streamOffset }: { groups: [number, PublicationProject[]][]; t: (key: string) => string; activeIndex: number; streamViewportRef: RefObject<HTMLDivElement | null>; streamRef: RefObject<HTMLDivElement | null>; streamOffset: number }) {
...
export default function Publications() {
  ...
  const visibleRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      ...
      const yearMatch = !filters.year || String(record.year) === filters.year;
      const topicMatch = !filters.topics.length || filters.topics.every((topic) => record.topics.includes(topic));
      const methodMatch = !filters.methods.length || filters.methods.every((method) => record.methods.includes(method));
      return textMatch && yearMatch && topicMatch && methodMatch;
    }).sort((a, b) => {
      const yearDifference = filters.sort === "newest" ? b.year - a.year : a.year - b.year;
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(grouped.entries()).sort(([a], [b]) => filters.sort === "newest" ? b - a : a - b);
  }, [filters.sort, visibleRecords]);
  const { activeIndex, streamOffset, streamTravel } = useStickyPublicationProgress(stickySectionRef, stickyViewportRef, streamViewportRef, streamRef, visibleRecords.length);
    ...
    <PublicContainer className="py-12 sm:py-16 lg:py-20">
      ...
      <div ref={stickySectionRef} style={stickyStyle} className={visibleRecords.length ? "publications-scroll-track relative" : ""}>
        ...
        <aside className="hidden border-r border-[var(--border)] pr-8 lg:block" aria-label={t("filterPublications")}>
          <PublicationFiltersPanel filters={filters} years={years} topics={topicOptions} methods={methodOptions} t={t} onChange={updateFilters} onClear={clearFilters} prefix="desktop" />
        </aside>
        ...
        <main className="mt-8 min-w-0 lg:mt-0 lg:flex lg:h-full lg:min-h-0 lg:flex-col">
          ...
          <input id="publication-search" type="search" value={filters.search} onChange={(event) => updateFilters({ search: event.target.value })} placeholder={t("searchPlaceholder")} className="mt-6 w-full border border-[var(--border)] bg-white px-4 py-3 text-sm text-[var(--navy)] outline-none placeholder:text-[var(--gray)] focus:border-[var(--rams-red)] focus:ring-2 focus:ring-[var(--rams-red)]/15" />

          {loading ? <div className="relative mt-8 space-y-6 pl-10 sm:pl-14"><div className="absolute bottom-0 left-3 top-0 w-px bg-[var(--border)]" />{[1, 2, 3].map((item) => <div key={item} className="relative"><span className="absolute -left-[2.15rem] top-8 h-3 w-3 border-2 border-[var(--rams-red)] bg-[var(--background-light)]" /> <PublicationSkeleton /></div>)}</div> : error ? <div className="mt-8"><PublicError message={t("error")} onRetry={load} /></div> : visibleRecords.length === 0 ? <div className="mt-8 border border-dashed border-[var(--border)] bg-white p-10 text-center"><p className="eyebrow text-[var(--ais-blue)]">{t("emptyEyebrow")}</p><h2 className="mt-3 font-display text-2xl font-semibold text-[var(--navy)]">{t("emptyTitle")}</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[var(--slate)]">{t("emptyDescription")}</p>{isActive(filters) && <button type="button" onClick={clearFilters} className="mt-6 text-sm font-semibold text-[var(--rams-red)] underline underline-offset-4">{t("clearAll")}</button>}</div> : <PublicationTimeline groups={groups} t={t} activeIndex={activeIndex} streamViewportRef={streamViewportRef} streamRef={streamRef} streamOffset={streamOffset} />}
        </main>
          </div>
        </div>
      </div>

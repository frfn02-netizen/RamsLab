  ...
  return (
    <header className="sticky top-0 z-50 border-b-2 border-[var(--ais-blue)] bg-white text-[var(--charcoal)] shadow-[0_4px_18px_rgba(11,32,56,0.08)]">
      <div className="mx-auto flex h-20 max-w-[1280px] items-center justify-between gap-6 px-5 sm:px-8 lg:px-10">
        ...
        >
          <div className="relative h-10 w-[80px] shrink-0">
            <Image
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[0.65rem] text-[var(--gray)]">
              {brand("technicalLine")}
            </span>
          </div>
        </Link>
          ...
          {links.map(([key, href]) =>
            href ? (
              <Link
                key={key}
                href={href}
                className={`text-base font-semibold transition ${isActive(pathname, href) ? "text-[var(--rams-red)]" : "text-[var(--charcoal)] hover:text-[var(--rams-red)]"}`}
              >
                {t(key)}
              </Link>
            ) : (
              <span
                key={key}
                aria-disabled="true"
                className="text-base font-semibold text-[var(--gray)]"
              >
                {t(key)}
              </span>
            ),
      ...
      {open && (
        <nav
          ...
          className="border-t border-[var(--ais-blue)] bg-[var(--background-light)] px-5 py-5 sm:px-8 lg:hidden"
            ...
            {links.map(([key, href]) =>
              href ? (
                <Link
                  ...
                  className={`py-1 text-base font-semibold transition ${isActive(pathname, href) ? "text-[var(--rams-red)]" : "text-[var(--charcoal)] hover:text-[var(--rams-red)]"}`}
                >
                  {t(key)}
                </Link>
              ) : (
                <span
                  key={key}
                  aria-disabled="true"
                  className="py-1 text-base font-semibold text-[var(--gray)]"
                >
                  {t(key)}
                </span>
              ),

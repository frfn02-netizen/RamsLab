...
import { createStudent, uploadStudentPhoto } from "@/lib/api/modules";
...
export default function CreateStudent() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const student = await createStudent({ fullName: form.fullName, studentType: form.studentType, program: form.program || undefined, specialization: form.specialization.split(",").map((item) => item.trim()).filter(Boolean), bio: form.bio || undefined, linkedin: form.linkedin || undefined, isPublic: form.isPublic });
      if (photoFile) await uploadStudentPhoto(student._id, photoFile);

...
export default function PublicationsPage() {
  ...
  return <div className="p-5 sm:p-7 lg:p-9"><div className="mx-auto max-w-7xl space-y-7">
    ...
    {loading ? <Card><LoadingState label="Loading publications" /></Card> : publications.length === 0 ? <EmptyState 
          ...
          action={user?.role === "ADMIN" && !search ? <LinkButton href="/dashboard/publications/new">Add publication</LinkButton> : undefined} /> : <Card><div 
          ...
          className="max-w-md px-5 py-4"><Link href={`/dashboard/publications/${publication._id}/edit`} 
          className="font-semibold hover:text-[var(--rams-red)]">{publication.title}</Link><p 
          ...
          className="flex justify-end gap-2">{user?.role === "ADMIN" && <><LinkButton href={`/dashboard/publications/${publication._id}/edit`} variant="secondary">Edit</LinkButton><Button 

...
function fromPublication(publication: Publication): FormState {
  return {
    ...
    publicationType: publication.publicationType || "Article",
...
export default function PublicationForm({ id }: { id?: string }) {
  ...
  function validate() {
    ...
    if (!form.authors.length) {
      next.authors = "Add at least one author.";
    }

    if (!form.publicationType.trim()) {
      next.publicationType = "Publication type is required.";
    }
    ...
    if (
      ...
      next.year = "Enter a valid publication year.";
    }
  ...
  function addAuthor() {
    const next = addValue(form.authors, authorDraft);

    if (next.length !== form.authors.length) {
      update("authors", next);
    }
  ...
  function addTopic() {
    const next = addValue(form.topics, topicDraft);

    if (next.length !== form.topics.length) {
      update("topics", next);
    }
  ...
  function addMethod() {
    const next = addValue(form.methods, methodDraft);

    if (next.length !== form.methods.length) {
      update("methods", next);
    }
  ...
  function onEnter(event: KeyboardEvent<HTMLInputElement>, add: () => void) {
    if (event.key === "Enter") {
      event.preventDefault();
      add();
    }
  ...
  if (
    !hasPermission(
      user?.role,
      editing ? "publication.update" : "publication.create",
    )
  ) {
    return (
      <div className="p-5 sm:p-7 lg:p-9">
        <div className="mx-auto max-w-3xl">
          <ErrorState message="You do not have permission to manage publications." />
        </div>
      </div>
    );
  }

  if (editing && audit && !canEditRecord) {
    return (
      <div className="p-5 sm:p-7 lg:p-9">
        <div className="mx-auto max-w-3xl">
          <ErrorState message="You can only edit publications you created." />
        </div>
      </div>
    );
  }
    ...
    <div className="p-5 sm:p-7 lg:p-9">
      ...
      <div className="mx-auto max-w-4xl space-y-7">
        ...
        <PageHeader
          ...
          description="Publication records are shared by the admin and public Publications pages."
        />
          ...
          <form onSubmit={submit} className="space-y-6">
            ...
            <Field label="Authors *" error={errors.authors}>
              <div className="flex flex-wrap gap-2">
                {form.authors.map((author) => (
                  <span
                    key={author}
                    className="inline-flex items-center gap-2 rounded-full bg-[var(--rams-gray-light)] px-3 py-1.5 text-sm"
                  >
                    {author}

                    <button
                      type="button"
                      className="font-bold text-[var(--rams-gray)] hover:text-[var(--rams-red)]"
                      onClick={() =>
                        update(
                          "authors",
                          form.authors.filter((item) => item !== author),
                        )
                      }
              ...
              <div className="mt-3 flex gap-2">
                <input
                  ...
                  onKeyDown={(event) => onEnter(event, addAuthor)}
                />
            ...
            >
              <select
                ...
                onChange={(event) =>
                  update("publicationType", event.target.value)
                }
              >
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <Field label="DOI" htmlFor="publication-doi">
                <input
                  id="publication-doi"
                  className={inputClass}
            ...
            <TagField
              ...
              remove={(value) =>
                update(
                  "topics",
                  form.topics.filter((item) => item !== value),
                )
              }
              onEnter={onEnter}
              placeholder="Hydrodynamics"
            />
            ...
            <TagField
              ...
              values={form.methods}
              draft={methodDraft}
              setDraft={setMethodDraft}
              add={addMethod}
              remove={(value) =>
                update(
                  "methods",
                  form.methods.filter((item) => item !== value),
                )
              }
              onEnter={onEnter}
              placeholder="CFD"
...
function PublicationAudit({ publication }: { publication: Publication }) {
  ...
  ) => email ?? (id ? `User ${id.slice(0, 8)}…` : "Unknown");

  return (
    <Card className="border-black/5 bg-[var(--rams-gray-light)] p-4 text-xs text-[var(--rams-gray)]">
      <div className="grid gap-2 sm:grid-cols-2">
        <p>
          Created by:{" "}
          <span className="font-semibold text-[var(--rams-charcoal)]">
            {identity(publication.createdByEmail, publication.createdBy)}
          </span>
        </p>
        ...
        <p>
          Last updated by:{" "}
          <span className="font-semibold text-[var(--rams-charcoal)]">
            {identity(publication.updatedByEmail, publication.updatedBy)}
          </span>
        </p>
...
function TagField({
  ...
  onEnter: (event: KeyboardEvent<HTMLInputElement>, add: () => void) => void;
  placeholder: string;
}) {
  return (
    <Field label={label}>
      ...
      <div className="mt-3 flex gap-2">
        <input
          ...
          onKeyDown={(event) => onEnter(event, add)}
        />

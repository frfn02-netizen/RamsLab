...
type FormErrors = Partial<Record<"title" | "authors" | "year" | "journal" | "pdfUrl", string>>;
...
export default function PublicationForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    getPublication(id).then((publication) => { if (!cancelled) 
      setForm(fromPublication(publication)); }).catch((reason) => 
  ...
  if (user && user.role !== "ADMIN") return <div className="p-5 sm:p-7 lg:p-9"><div className="mx-auto max-w-3xl"><ErrorState message="Only administrators can manage publications." /></div></div>;
  ...
  return <div className="p-5 sm:p-7 lg:p-9"><div className="mx-auto max-w-4xl space-y-7">
    ...
    <Card className="p-6"><form onSubmit={submit} 
      ...
      <Field label="Publication Type *" htmlFor="publication-type"><select id="publication-type" required className={inputClass} value={form.publicationType} onChange={(event) => update("publicationType", event.target.value)}>{PUBLICATION_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></Field>
      <div className="grid gap-5 sm:grid-cols-2"><Field label="Year *" htmlFor="publication-year" error={errors.year}><input id="publication-year" required type="number" min="1900" max={new Date().getFullYear() + 1} step="1" className={inputClass} value={form.year} onChange={(event) => update("year", event.target.value)} /></Field><Field label="Journal *" htmlFor="publication-journal" error={errors.journal}><input id="publication-journal" required className={inputClass} value={form.journal} onChange={(event) => update("journal", event.target.value)} /></Field><Field label="DOI" htmlFor="publication-doi"><input id="publication-doi" className={inputClass} placeholder="10.0000/example" value={form.doi} onChange={(event) => update("doi", event.target.value)} /></Field><Field label="PDF URL" htmlFor="publication-pdf" error={errors.pdfUrl}><input id="publication-pdf" type="url" className={inputClass} placeholder="https://example.org/paper.pdf" value={form.pdfUrl} onChange={(event) => update("pdfUrl", event.target.value)} /></Field></div>

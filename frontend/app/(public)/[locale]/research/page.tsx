...
export default async function ResearchPage({
  ...
  return (
    <>
      ...
      <section className="bg-white py-16 sm:py-24">
        <PublicContainer className="grid items-center gap-14 lg:grid-cols-2 lg:gap-20">
          <div className="hero-entrance max-w-2xl">
            ...
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-[var(--gray)]">
              {t("heroDescription")}
            </p>
      ...
      <section
        id="areas"
        className="scroll-mt-20 bg-[var(--background-light)] py-20 sm:py-28"
      >
        <PublicContainer>
          <RevealOnScroll className="max-w-2xl">
            <p className="eyebrow">{t("indexEyebrow")}</p>

            <h2 className="mt-4 font-display text-3xl font-bold tracking-[-0.02em] text-[var(--navy)] sm:text-4xl">
              {t("indexTitle")}
            </h2>
          ...
          <RevealOnScroll className="mt-14" stagger={90}>
            {researchUnavailable ? (
              ...
              <div className="border border-dashed border-[var(--border)] bg-white p-9 text-center">
              ...
              <ol className="reveal-stagger-list border-t border-[var(--border)]">
                {researchAreas.map((area, index) => {
                  ...
                  return (
                    <li
                      key={area.code}
                      className="border-b border-[var(--border)] transition hover:bg-white"
                    >
                      <Link
                        href={`#${area.code.toLowerCase()}`}
                        className="group grid gap-3 py-8 sm:grid-cols-[5.5rem_1fr_auto] sm:items-baseline sm:gap-8 sm:py-10"
                      >
                        ...
                        <span
                          className="hidden text-sm font-semibold text-[var(--gray)] transition group-hover:text-[var(--rams-red)] sm:block"
                          aria-hidden="true"
                        >
                          {t("details")} →
                        </span>
            ...
            {researchAreas.map((area, index) => {
              ...
              return (
                <article
                  ...
                  className="scroll-mt-20 grid gap-10 border-t border-[var(--border)] py-16 lg:grid-cols-2 lg:gap-20 lg:py-20"
                >
                  <div>
                    <p className="flex items-baseline gap-3">
                      <span className="font-display text-xl font-bold text-[var(--rams-red)]">
                        0{index + 1}
                      </span>
                  ...
                  <div className="lg:border-l lg:border-[var(--border)] lg:pl-12">
                      ...
                      <ul className="mt-6 space-y-4">
                        {content.methods.map((method) => (
                          <li key={method} className="flex items-start gap-3">
                            <span
                              className="mt-1.5 h-1.5 w-1.5 shrink-0 bg-[var(--rams-red)]"
                              aria-hidden="true"
                            />

                            <span className="text-sm leading-6 text-[var(--navy)]">
                              {method}
                            </span>
                          </li>
                        ))}
                    ...
                    <div className="mt-10 border-t border-[var(--border)] pt-8">
                      ...
                      <p className="mt-4 font-display text-lg font-semibold tracking-wide text-[var(--navy)]">
      ...
      <section className="bg-[var(--background-light)] py-20 sm:py-28">
        <PublicContainer>
          <RevealOnScroll className="max-w-2xl">
            ...
            <p className="mt-4 text-lg leading-relaxed text-[var(--gray)]">
          ...
          <RevealOnScroll className="mt-14" stagger={100}>
            <ol className="reveal-stagger-list grid gap-px border border-[var(--border)] bg-[var(--border)] sm:grid-cols-2 lg:grid-cols-4">
              {(["understand", "analyse", "model", "decide"] as const).map(
                (step, index) => (
                  <li key={step} className="relative bg-white p-8">
                    {index < 3 && (
                      <span
                        className="absolute right-5 top-1/2 z-10 hidden -translate-y-1/2 text-2xl font-light text-[var(--gray)] lg:block"
                        aria-hidden="true"
                      >
                        →
                      </span>
                    ...
                    <span className="font-display text-lg font-bold text-[var(--rams-red)]">
                      0{index + 1}
                    </span>

                    <h3 className="mt-5 font-display text-xl font-bold text-[var(--navy)]">
              ...
              <ul className="mt-10 grid gap-x-8 gap-y-4 border-t border-[var(--border)] pt-8 sm:grid-cols-2">
                {(
                  ...
                  <li key={item} className="flex items-center gap-3">
                    <span
                      className="h-1.5 w-1.5 shrink-0 bg-[var(--rams-red)]"
                      aria-hidden="true"
                    />
        ...
        <PublicContainer>
          ...
          >
            ...
            <ol className="border-t border-white/15">
              {(
                ["reliability", "safety", "maintenance", "decision"] as const
              ).map((point, index) => (
                <li
                  key={point}
                  className="flex gap-6 border-b border-white/15 py-6"
                >
                  <span className="pt-1 font-display text-sm font-bold text-[var(--rams-red)]">
                    0{index + 1}
                  </span>

                  <p className="font-display text-lg font-semibold leading-snug text-white/90">
                    {t(`industryPoints.${point}`)}
                  </p>
                </li>
              ))}
            </ol>
        ...
        <PublicContainer>
          <RevealOnScroll className="mx-auto max-w-3xl text-center">
            ...
            <div className="mt-10 flex flex-col justify-center gap-4 sm:flex-row">
              ...
              <Link
                href="/about"
                className="border border-white/40 px-8 py-3 text-sm font-semibold text-white transition hover:border-white hover:bg-white/10"
              >
                {t("ctaSecondary")} →
              </Link>
            </div>

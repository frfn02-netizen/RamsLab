    ...
    <div className="p-5 sm:p-7 lg:p-9">
      <div className="mx-auto max-w-7xl space-y-7">
        <PageHeader
          ...
          action={user?.role === "ADMIN" ? <LinkButton href="/dashboard/projects/new">Add project</LinkButton> : undefined}
        />

        <Card className="flex flex-wrap gap-2 p-4">
          {categories.map((item) => (
            <Button key={item} variant={category === item ? "primary" : "secondary"} onClick={() => setCategory(item)}>
        ...
        {loading ? <Card><LoadingState label="Loading projects" /></Card> : visibleProjects.length === 0 ? (
          <EmptyState title="No projects found" description="There are no project records matching this view yet." action={user?.role === "ADMIN" ? <LinkButton href="/dashboard/projects/new">Create a project</LinkButton> : undefined} />
        ) : (
          <Card>
              ...
              <table className="w-full min-w-[760px] text-left">
                ...
                <tbody className="divide-y divide-black/8">
                  {visibleProjects.map((project) => (
                    <tr key={project._id}>
                      <td className="px-5 py-4"><Link href={`/dashboard/projects/${project._id}`} className="font-semibold hover:text-[var(--rams-red)]">{project.title}</Link><p className="mt-1 text-xs text-[var(--rams-gray)]">{project.slug}</p></td>
                      <td className="px-5 py-4"><Badge tone="red">{label(project.category)}</Badge></td>

...
async function seedDosen() {
  try {
    ...
    if (!email || !password || password.length < 12) {
      throw new Error(
        "SEED_DOSEN_EMAIL and a 12+ character SEED_DOSEN_PASSWORD are required",
      );
    }

    const existingDosen = await users.findOne({
      email,
    });
    ...
    console.log("✅ Dosen user created");

    console.log("Dosen account created");
  } catch (error) {
    console.error("❌ Failed to seed dosen:", error);

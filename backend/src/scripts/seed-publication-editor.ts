...
async function seedPublicationEditor() {
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "The development Publication Editor seed is disabled in production",
    );
  }
  ...
  if (!email || !password || password.length < 12) {
    throw new Error(
      "SEED_PUBLICATION_EDITOR_EMAIL and a 12+ character SEED_PUBLICATION_EDITOR_PASSWORD are required",
    );
  }

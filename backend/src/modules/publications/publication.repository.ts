...
export class PublicationConflictError extends Error {
  constructor(
    message = "A publication with the same DOI or normalized title and year already exists",
  ) {
    super(message);
...
function duplicateFilter(
  publication: { doi?: string | null; normalizedTitle: string; year: number },
  excludeId?: string,
) {
...
async function ensureNoDuplicate(
  ...
  const duplicate = await getPublicationsCollection().findOne(
    duplicateFilter(publication, excludeId),
    { projection: { _id: 1 } },
  );
  if (duplicate) {
...
function toDocument(
  ...
  const now = new Date();
  const has = (key: keyof CreatePublicationInput) =>
  ...
  return {
    ...
    createdBy: existing
      ...
      : new ObjectId(actor.userId),
    updatedBy: existing ? new ObjectId(actor.userId) : null,
    normalizedTitle: normalizePublicationTitle(
...
export async function findAllPublications(options?: {
  search?: string;
  year?: number;
  topics?: string[];
  methods?: string[];
  ...
  const limit = Math.min(
    ...
    Math.max(1, options?.limit ?? SECURITY_LIMITS.maxPageSize),
  );
...
export function canModifyPublication(
  ...
  return (
    actor.role === "ADMIN" || publication.createdBy?.toString() === actor.userId
  );
}
...
export async function updatePublication(
  ...
  return getPublicationsCollection().findOneAndUpdate(

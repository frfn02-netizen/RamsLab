...
import type {
  CreateResearchAreaInput,
  UpdateResearchAreaInput,
...
export async function findAllResearchAreas(options?: {
...
export async function countResearchAreas(options?: {
  publishedOnly?: boolean;
}): Promise<number> {
  const filter = options?.publishedOnly ? { published: true } : {};
...
export async function findResearchAreaById(
  id: string,
...
export async function findResearchAreaByCodeOrSlug(
  code: string,
  slug: string,
): Promise<ResearchArea | null> {
...
export async function deleteResearchArea(id: string): Promise<boolean> {
  ...
  const result = await getResearchAreasCollection().deleteOne({
    _id: new ObjectId(id),
  });

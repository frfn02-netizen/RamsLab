...
import { findAllSiteContent, findSiteContentByKey, updateSiteContent } from "./site-content.repository.js";
import { SITE_CONTENT_KEYS, type SiteContentContent, type SiteContentDocument, type SiteContentKey } from "./site-content.types.js";
...
export async function getSiteContentListController(_req: Request, res: Response) {
  try {
    const documents = await findAllSiteContent();
    return res.json({ success: true, data: documents.map(adminRepresentation) });
  } catch {
...
export async function getSiteContentController(req: Request, res: Response) {
  ...
  if (!isSiteContentKey(key)) return res.status(400).json({ success: false, message: "Invalid site content key" });
  try {
    const document = await findSiteContentByKey(key);
    if (!document) return res.status(404).json({ success: false, message: "Site content not found" });
...
export async function updateSiteContentController(req: Request, res: Response) {
  ...
  if (!isSiteContentKey(key)) return res.status(400).json({ success: false, message: "Invalid site content key" });
  try {
    const content = siteContentSchemas[key].parse(req.body?.content) as SiteContentContent;
...
export async function getPublicSiteContentController(req: Request, res: Response) {
  const key = req.params.key as string;
  if (!isSiteContentKey(key)) return res.status(400).json({ success: false, message: "Invalid site content key" });
  try {
    const document = await findSiteContentByKey(key);
    ...
    return res.status(500).json({ success: false, message: "Failed to fetch public site content" });
  }
}

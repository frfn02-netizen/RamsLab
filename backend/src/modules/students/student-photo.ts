...
export async function removeStudentPhoto(photo?: string) {
  if (!photo) return;
  let filename: string;
  try {
...
export function getStudentPhotoUrl(req: Request, filename: string) {
  return `${getPublicApiOrigin(req)}/uploads/students/${filename}`;
}

...
beforeAll(async () => {
  ...
  if (!secret) {
    throw new Error(
      "JWT_SECRET is required for dosen tests"
    );
  }
...
describe("Dosen API", () => {
    ...
    const response =
      await request(app)
        ...
        .send({
          ...
          phone: "081234567890",
          photo:
            "https://example.com/vitest-dosen.jpg",
          bio:
            "Dosen created specifically for automated testing.",
          linkedin:
            "https://www.linkedin.com/in/vitest-dosen",
          isPublic: true,
        });
  ...
  it("should get dosen by employee id", async () => {
    const response =
      await request(app)
        .get(
          `/api/dosen/employee/${TEST_EMPLOYEE_ID}`
        )
        .set(
          "Cookie",
          `rams_access_token=${adminToken}`

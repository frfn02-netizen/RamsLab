...
import { getTrackingCollection } from "../src/modules/tracking/tracking.respository.js";
import {
  ensureTestUsers,
  signTestToken,
  TEST_ADMIN_USER_ID,
...
const TEST_ALUMNI_ID = new ObjectId();

const TEST_NIM = "VITEST-TRACKING-001";

const TEST_TRACKING_TITLE = "Vitest Tracking";
...
beforeAll(async () => {
  ...
  if (!secret) {
    throw new Error("JWT_SECRET is required for tracking tests");
  }
  ...
  const trackingCollection = getTrackingCollection();
...
describe("Tracking API", () => {
  it("should create alumni tracking", async () => {
    const response = await request(app)
      .post(`/api/tracking/alumni/${TEST_ALUMNI_ID.toHexString()}`)
      ...
      .send({
        ...
        endDate: null,
        description: "Tracking record created for automated testing.",
      });
    ...
    expect(response.body.data.title).toBe(TEST_TRACKING_TITLE);

    expect(response.body.data.alumniId).toBe(TEST_ALUMNI_ID.toHexString());

    trackingId = response.body.data._id;
  });

  it("should get tracking by alumni id", async () => {
    const response = await request(app)
      .get(`/api/tracking/alumni/${TEST_ALUMNI_ID.toHexString()}`)
    ...
    expect(
      response.body.data.some(
        (tracking: any) => tracking.title === TEST_TRACKING_TITLE,
      ),
    ).toBe(true);
  });
  ...
  it("should get tracking by id", async () => {
    ...
    expect(response.body.data._id).toBe(trackingId);
  ...
  it("should reject invalid alumni id", async () => {
    const response = await request(app)
      .get("/api/tracking/alumni/invalid-id")
      .set("Cookie", `rams_access_token=${dosenToken}`);
    ...
    expect(response.body.message).toBe("Invalid alumni ID");
  });

  it("should reject invalid tracking id", async () => {
    const response = await request(app)
      .get("/api/tracking/invalid-id")
      .set("Cookie", `rams_access_token=${dosenToken}`);
  ...
  it("should reject create tracking with invalid alumni id", async () => {
    const response = await request(app)
      .post("/api/tracking/alumni/invalid-id")
      .set("Cookie", `rams_access_token=${adminToken}`)
      .send({
        type: "EMPLOYMENT",
  ...
  it("should update alumni tracking", async () => {
    ...
    expect(response.body.data.position).toBe("Senior Software Engineer");
  ...
  it("should reject invalid tracking update id", async () => {
    ...
    expect(response.body.message).toBe("Invalid tracking ID");
  });
  ...
  it("should delete alumni tracking", async () => {
    ...
    expect(response.body.message).toBe("Tracking deleted successfully");

    const deleted = await getTrackingCollection().findOne({
  ...
  it("should reject invalid tracking delete id", async () => {
    const response = await request(app)
      .delete("/api/tracking/invalid-id")
      .set("Cookie", `rams_access_token=${adminToken}`);

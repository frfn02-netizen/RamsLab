...
const TEST_UNIVERSITY_NAME =
  "Vitest University Partner";

const TEST_INDUSTRIAL_NAME =
  "Vitest Industrial Partner";
...
beforeAll(async () => {
  ...
  if (!secret) {
    throw new Error(
      "JWT_SECRET is required for partner tests"
    );
  }
...
describe("Partners API", () => {
    ...
    const response =
      await request(app)
        ...
        .send({
          ...
          country: "Indonesia",
          description:
            "University partner for automated testing.",
          isFeatured: true,
          published: true,
        });
  ...
  it("should get the university partner", async () => {
    const response =
      await request(app)
        .get(
          `/api/partners/university/${universityId}`
        )
        .set(
          "Cookie",
          `rams_access_token=${adminToken}`
  ...
  it("should update the university partner", async () => {
    ...
    ).toBe("Updated by Vitest.");

    expect(
      response.body.data.isFeatured
    ).toBe(false);
  });
  ...
  it("should reject invalid university partner id", async () => {
    const response =
      await request(app)
        .get(
          "/api/partners/university/invalid-id"
        )
        .set(
          "Cookie",
          `rams_access_token=${adminToken}`
    ...
    const response =
      await request(app)
        ...
        .send({
          ...
          country: "Indonesia",
          description:
            "Industrial partner for automated testing.",
          isFeatured: true,
          published: true,
        });
  ...
  it("should get the industrial partner", async () => {
    const response =
      await request(app)
        .get(
          `/api/partners/industrial/${industrialId}`
        )
        .set(
          "Cookie",
          `rams_access_token=${adminToken}`
